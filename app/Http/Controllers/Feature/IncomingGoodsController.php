...
class IncomingGoodsController extends Controller
{
    ...
    {
        $receipts = $this->incomingGoodsService->getAllReceipts();
    ...
    public function storeStep1(Step1Request $request)
    {
        ...
        return redirect()->route('incoming-goods.step2')
            ->with('success', 'Data penerimaan berhasil disimpan. Lanjut ke input berat nota.');
    }
    ...
    public function createStep2()
    {
        ...
        if (!session()->has('step1_data')) {
            return redirect()->route('incoming-goods.step1')
                ->with('error', 'Silakan lengkapi data penerimaan terlebih dahulu');
        }
    ...
    public function storeStep2(Step2Request $request)
    {
        ...
        return redirect()->route('incoming-goods.step3')
            ->with('success', 'Data berat nota berhasil disimpan. Lanjut ke input timbangan gudang.');
    }
    ...
    public function storeFinal(Step3Request $request)
    {
        ...
        try {
            ...
            // Save to database
            $receipt = $this->incomingGoodsService->createPurchaseReceipt(
                $step1Data,
                $step2Data,
                $step3Data
            );
            ...
            return redirect()->route('incoming-goods.show', $receipt->id)
    ...
    {
        $receipt = \App\Models\PurchaseReceipt::with(['supplier', 'receiptItems.gradeSupplier'])
            ->findOrFail($id);
