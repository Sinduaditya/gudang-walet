       ...
       <div id="progressSteps" class="mb-8 bg-white rounded-lg shadow-sm border p-6">
            ...
            <div class="flex items-center justify-between max-w-3xl mx-auto">
                <div class="flex flex-col items-center flex-1">
                    <div class="w-10 h-10 flex items-center justify-center rounded-full bg-purple-500 text-white font-semibold text-base shadow-sm">
                        1
                    </div>
                            ...
                            <div class="relative">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div>
                                        ...
                                        @php
                                            $gudangUtama = $locations->where('name', 'Gudang Utama')->first()
                                                        ?? ($locations->where('id', 1)->first() ?? $locations->first());
                                        @endphp

                                        <input type="hidden" name="from_location_id" value="{{ $gudangUtama->id ?? '' }}">
                                        ...
                                        <div class="w-full border-2 border-gray-300 bg-gray-50 rounded-lg p-3 text-gray-700 flex items-center">
                                            ...
                                            <span class="font-semibold">{{ $gudangUtama->name ?? 'Gudang Utama' }}</span>
                                        </div>
                                ...
                                @forelse($transferInternalTransactions as $tx)
                                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right">
                                            <div class="text-sm font-semibold {{ $tx->quantity_change_grams > 0 ? 'text-green-600' : 'text-red-600' }}">
                                                {{ $tx->quantity_change_grams > 0 ? '+' : '' }}{{ number_format(abs($tx->quantity_change_grams), 2) }} gr
                                            </div>
                                    ...
                                    <tr>
                                        <td colspan="5" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center">
                                                ...
                                                <p class="text-gray-500 font-medium">Belum ada daftar transfer internal</p>
                                                <p class="text-gray-400 text-sm mt-1">Transaksi akan muncul setelah Anda melakukan transfer</p>
                                            </div>
                                        </td>
                                    </tr>
                                @endforelse
                        ...
                        <div class="px-6 py-4 border-t border-gray-200 bg-gray-50">
                            <div class="flex items-center justify-between">
                                <div class="text-sm text-gray-600">
                                    Menampilkan {{ $transferInternalTransactions->firstItem() }} - {{ $transferInternalTransactions->lastItem() }} dari {{ $transferInternalTransactions->total() }} transaksi
                                </div>
                                <div>
